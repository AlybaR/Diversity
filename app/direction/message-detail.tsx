...
export default function DirectionMessageDetailScreen() {
  ...
  // Garde-fou : pas de fuite si la direction n'a pas le scope du message.
  if (message && !canRoleSeeScope('direction', message.visibilityScope)) {
    return (
      <View className="flex-1 bg-slate-50">
        ...
        <View className="flex-1 items-center justify-center px-8">
          ...
          <SecondaryButton
            label="Retour à mes messages"
            onPress={() => router.replace('/direction/messages' as Href)}
          />
        </View>
    ...
    <View className="flex-1 bg-slate-50">
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 140 }}>
        <View className="bg-white rounded-2xl p-5 border border-slate-100 mb-3">
          ...
          {!message.lu && (
            <View className="self-start bg-direction-50 rounded-full px-3 py-1 mb-4">
              <Text className="text-direction-700 text-xs font-semibold">Non lu</Text>
            </View>
          )}
          ...
          <View className="bg-white rounded-2xl p-4 border border-slate-100 mb-3">
            ...
            <View className="gap-2">
              {message.piecesJointes.map((piece) => (
                ...
                >
                  <View className="w-9 h-9 bg-direction-50 rounded-lg items-center justify-center">
